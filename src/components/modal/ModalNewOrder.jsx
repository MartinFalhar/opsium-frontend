...
export default function ModalNewOrder({
  ...
  const { item: pluItem, loading: pluLoading, error: pluError, getPluItem } = useStoreGetPluItem();

  // Hook pro načtení obruby podle PLU
  const { frame: pluFrame, loading: frameLoading, error: frameError, getPluFrame } = useStoreGetPluFrame();
  ...
  const [orderNoticeSelection, setOrderNoticeSelection] = useState(["SMS", "E-mail"]);
  ...
  const handleAddGlasses = () => {
    const newGlasses = {
      right: { sph: '', cyl: '', osa: '', add: '', prizma: '', baze: '', pd: '', vyska: '', vertex: '', panto: '' },
      left: { sph: '', cyl: '', osa: '', add: '', prizma: '', baze: '', pd: '', vyska: '', vertex: '', panto: '' },
      pbs: '',
      obruby: '',
      zabrus: '',
      brylove_cocky: ''
  ...
  const handleRemoveGlasses = (index) => {
    ...
    setPaymentItems((prev) => prev.filter(item => item.glassesIndex !== index));
  };
  ...
  useEffect(() => {
    ...
    setPaymentItems(prevItems => {
      const glassesPayments = prevItems.filter(item => item.glassesIndex !== undefined);
          ...
          <div className="order-information">
            ...
            <div className="order-information-attributes box">
              ...
              <h3>Poslat poštou na:</h3>
                  ...
                  onChange={(e) => handleChange("address_for_delivery", e.target.value)}
          ...
          <div className="order-obligatory-items">
                <div style={{ display: 'flex', gap: '10px', marginBottom: '10px', alignItems: 'center' }}>
                  <input
                    ...
                    placeholder="PLU"
                  />
                  <button type="button" onClick={handleAddGlasses} style={{ padding: '8px 12px', whiteSpace: 'nowrap', width: 'auto', height: 'fit-content' }}>+ BRÝLE</button>
                  <button type="button" style={{ padding: '8px 12px', whiteSpace: 'nowrap', width: 'auto', height: 'fit-content' }}>+ KONTAKTNÍ ČOČKY</button>
                  <button type="button" style={{ padding: '8px 12px', whiteSpace: 'nowrap', width: 'auto', height: 'fit-content' }}>+ SERVIS</button>
                </div>
                ...
                {obligatoryItems.length > 0 && (
                  <div style={{ marginTop: '20px' }}>
                    {obligatoryItems.map((item, index) => (
                      ...
                      >
                        <span><strong>PLU:</strong> {item.plu}</span>
                        <span><strong>Model:</strong> {item.model}</span>
                        <span><strong>Velikost:</strong> {item.size}</span>
                        <span><strong>Barva:</strong> {item.color}</span>
                        <span><strong>MJ:</strong> {item.uom}</span>
                        <span><strong>Cena:</strong> {item.price} Kč</span>
                        <span style={{ fontSize: '0.7em' }}><strong>DPH:</strong> {item.rate}%</span>
                  ...
                  return (
                    ...
                    >
                      {isCollapsed ? (
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gridTemplateRows: 'auto auto', gap: '10px', alignItems: 'center' }}>
                          <strong style={{ width: '80px' }}>Pravé oko:</strong>
                          <div style={{ display: 'flex', gap: '10px' }}>
                            <input
                              type="text"
                              value={glasses.right.sph}
                              onChange={(e) => handleGlassesChange(index, 'right', 'sph', e.target.value)}
                              placeholder="SPH"
                              style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                            />
                            <input
                              type="text"
                              value={glasses.right.cyl}
                              onChange={(e) => handleGlassesChange(index, 'right', 'cyl', e.target.value)}
                              placeholder="CYL"
                              style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                            />
                            <input
                              ...
                              onChange={(e) => handleGlassesChange(index, 'right', 'osa', e.target.value)}
                              placeholder="OSA"
                              style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                            />
                            <input
                              ...
                              onChange={(e) => handleGlassesChange(index, 'right', 'add', e.target.value)}
                              placeholder="ADD"
                              style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                            />
                            {expandedDioptrie[index] && (
                              <>
                                <input
                                  ...
                                  onChange={(e) => handleGlassesChange(index, 'right', 'prizma', e.target.value)}
                                  placeholder="PRIZMA"
                                  style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                                />
                                <input
                                  ...
                                  onChange={(e) => handleGlassesChange(index, 'right', 'baze', e.target.value)}
                                  placeholder="BÁZE"
                                  style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                                />
                              </>
                          ...
                          <strong style={{ width: '80px' }}>Levé oko:</strong>
                          <div style={{ display: 'flex', gap: '10px' }}>
                            <input
                              type="text"
                              value={glasses.left.sph}
                              onChange={(e) => handleGlassesChange(index, 'left', 'sph', e.target.value)}
                              placeholder="SPH"
                              style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                            />
                            <input
                              ...
                              onChange={(e) => handleGlassesChange(index, 'left', 'cyl', e.target.value)}
                              placeholder="CYL"
                              style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                            />
                            <input
                              ...
                              onChange={(e) => handleGlassesChange(index, 'left', 'osa', e.target.value)}
                              placeholder="OSA"
                              style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                            />
                            <input
                              ...
                              onChange={(e) => handleGlassesChange(index, 'left', 'add', e.target.value)}
                              placeholder="ADD"
                              style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                            />
                            {expandedDioptrie[index] && (
                              <>
                                <input
                                  ...
                                  onChange={(e) => handleGlassesChange(index, 'left', 'prizma', e.target.value)}
                                  placeholder="PRIZMA"
                                  style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                                />
                                <input
                                  ...
                                  onChange={(e) => handleGlassesChange(index, 'left', 'baze', e.target.value)}
                                  placeholder="BÁZE"
                                  style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                                />
                              </>
                        ...
                        <div>
                          ...
                          <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                            <input
                              ...
                              onChange={(e) => handleGlassesChange(index, 'right', 'pd', e.target.value)}
                              placeholder="PD"
                              style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                            />
                            <input
                              ...
                              onChange={(e) => handleGlassesChange(index, 'right', 'vyska', e.target.value)}
                              placeholder="Výška"
                              style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                            />
                            {expandedCentration[index] && (
                              <>
                                <input
                                  ...
                                  onChange={(e) => handleGlassesChange(index, 'right', 'vertex', e.target.value)}
                                  placeholder="Vertex"
                                  style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                                />
                                <input
                                  ...
                                  onChange={(e) => handleGlassesChange(index, 'right', 'panto', e.target.value)}
                                  placeholder="Panto"
                                  style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                                />
                              </>
                          ...
                          <div style={{ display: 'flex', gap: '10px' }}>
                            <input
                              ...
                              onChange={(e) => handleGlassesChange(index, 'left', 'pd', e.target.value)}
                              placeholder="PD"
                              style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                            />
                            <input
                              ...
                              onChange={(e) => handleGlassesChange(index, 'left', 'vyska', e.target.value)}
                              placeholder="Výška"
                              style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                            />
                            {expandedCentration[index] && (
                              <>
                                <input
                                  ...
                                  onChange={(e) => handleGlassesChange(index, 'left', 'vertex', e.target.value)}
                                  placeholder="Vertex"
                                  style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                                />
                                <input
                                  ...
                                  onChange={(e) => handleGlassesChange(index, 'left', 'panto', e.target.value)}
                                  placeholder="Panto"
                                  style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                                />
                              </>
                        ...
                        {expandedCentration[index] && (
                          <div style={{ display: 'flex', alignItems: 'center' }}>
                            <input
                              type="text"
                              value={glasses.pbs}
                              onChange={(e) => handleGlassesChange(index, 'shared', 'pbs', e.target.value)}
                              placeholder="PBS"
                              style={{ width: '80px', fontSize: '0.9em', fontWeight: 'bold', padding: '0 5px' }}
                            />
                          </div>
